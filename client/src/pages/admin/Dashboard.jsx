      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
            <div className="w-2 h-6 bg-indigo-600 rounded-sm"></div> Admin
            Overview
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            System metrics and recent activity across your organization.
          </p>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
          <StatsCard
            ...
            icon={FolderKanban}
            colorClass="bg-white border-t-2 border-t-indigo-600 shadow-md"
            textClass="text-slate-900"
            iconColor="text-indigo-600 bg-indigo-50"
          />
          <StatsCard
            title="Open"
            value={projects?.filter((p) => p.status === "open").length || 0}
            icon={FolderOpen}
            colorClass="bg-white hover:border-slate-300 transition-colors"
