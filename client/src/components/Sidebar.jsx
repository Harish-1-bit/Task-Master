...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-slate-900/50 z-40 lg:hidden backdrop-blur-[2px] transition-all"
          onClick={() => setIsOpen(false)}
        />
      )}

      <aside
        className={`w-64 min-h-screen bg-white border-r border-slate-200 flex flex-col fixed left-0 top-0 z-50 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}
      >
        {/* Brand */}
        <div className="h-20 flex items-center justify-between px-6 border-b border-slate-100 shrink-0">
          ...
          <button
            onClick={() => setIsOpen(false)}
            className="lg:hidden p-2 -mr-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <nav className="flex-1 px-3 space-y-1 overflow-y-auto">
          <p className="px-3 text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-4 mt-2">
            Main Menu
          </p>
          {items.map((item) => {
